...
public:
    ...
    std::vector<ProductionAction> onTopActions = {
        ...
        {
            {3004}, [&](Token &t) {
                logsStream << "Found endif keyword in If Statement, removing MFF" << std::endl;
                asserter->operatorsStack.pop();
            }
        },
        {
        ...
        {
            {3101}, [&](Token &t) {
                logsStream << "Got end of condition of while, adding SF without destiny" << std::endl;
                asserter->quadruples.push_back(new Asserter::SFQuadruple(asserter->varStack.pop(), -1));
            }
        },
        {
            {3102}, [&](Token &t) {
                logsStream << "Got endwhile keyword, removing MFF and replacing SF to " << asserter->jumpStack.top()
                        << std::endl;
                asserter->operatorsStack.pop();
                if (const auto sf = dynamic_cast<Asserter::SFQuadruple *>
                ...
                asserter->errors.emplace_back("FATAL: instruction " + std::to_string(asserter->jumpStack[0]) +
                                              " wasn't an SF!");
            }
        }
    };
